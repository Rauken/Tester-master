  ...
  document.getElementById('people').innerHTML = response.data.map(function (person) {
    return (
        '<div class="user-list container">' +
            '<span class="row bg-faded d-flex align-items-center border mt-1">' +
                '<div class="col-4 d-flex align-items-center border show">' +
                    '<img src="' + person.photo + '" class="rounded-circle" width="100" height="100"/>' +
                    '<h3 class="m-3">' + person.name + '</h3>' +
                    '<a href="#" id="eliminar" class="text-hide">Eliminar</a>' +    
                '</div>' +
                '<div class="col-8">' +
                    '<p class="m-1">' + person.description + '</p>' +
                '</div>' +
            '</span>' +
        '</div>'
    );
